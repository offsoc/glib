                                                    ...
                                                    const void        *unused)
{
  ...
  g_assert_nonnull (gi_callable_info_get_async_function (callable_info));
  g_assert_null (gi_callable_info_get_sync_function (callable_info));
  g_assert_null (gi_callable_info_get_finish_function (callable_info));

  GICallableInfo *async_info = gi_callable_info_get_async_function (callable_info);

  g_assert_null (gi_callable_info_get_async_function (async_info));
  g_assert_nonnull (gi_callable_info_get_sync_function (async_info));
  g_assert_nonnull (gi_callable_info_get_finish_function (async_info));

  g_assert_cmpstr (gi_base_info_get_name ((GIBaseInfo *) async_info), ==, "load_contents_async");

  GICallableInfo *sync_info = gi_callable_info_get_sync_function (async_info);

  g_assert_nonnull (gi_callable_info_get_async_function (sync_info));
  g_assert_null (gi_callable_info_get_finish_function (sync_info));
  g_assert_null (gi_callable_info_get_sync_function (sync_info));
